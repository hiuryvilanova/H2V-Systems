...
export default function Process() {
  ...
  return (
    <section id="processo" className="py-20 sm:py-[100px] lg:py-[120px] relative overflow-hidden">
      <div aria-hidden="true" className="absolute inset-0 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse 60% 40% at 50% 50%, rgba(232,75,26,0.07) 0%, transparent 60%)' }} />
      ...
      <div className="max-w-[1200px] mx-auto px-5 sm:px-6 relative z-10">
        <motion.div className="text-center mb-14 sm:mb-20"
          ...
          <h2 className="text-[clamp(1.75rem,5vw,3rem)] font-black tracking-tight">
            {t('title')} <span className="gradient-text">{t('titleHighlight')}</span>
          </h2>
          <p className="section-subtitle max-w-[500px] mx-auto px-2">{t('subtitle')}</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-10 relative">
          <div aria-hidden="true"
            ...
            style={{ background: 'linear-gradient(90deg, transparent, var(--cyan), var(--blue), var(--cyan), transparent)' }} />

          {steps.map((s, i) => (
            <motion.div key={i} className="text-center px-2 sm:px-4"
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.15 }}>
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center font-mono text-sm sm:text-base font-bold mx-auto mb-5 sm:mb-6 relative z-10"
