  ...
  return (
    <>
      ...
      <main id="main" tabIndex={-1}>
        {/* Hero */}
        <section className="relative pt-[120px] sm:pt-[140px] pb-12 sm:pb-16 overflow-hidden">
          <div aria-hidden="true" className="absolute inset-0 pointer-events-none"
            style={{ background: 'radial-gradient(ellipse 60% 50% at 70% 40%, rgba(232,75,26,0.14) 0%, transparent 60%)' }} />

          <div className="max-w-[1100px] mx-auto px-5 sm:px-6 relative z-10">
            {/* Breadcrumb */}
            <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs mb-6 sm:mb-8" style={{ color: 'var(--text-70)' }}>
            ...
            <h1 className="text-[clamp(1.9rem,5.5vw,3.4rem)] font-black tracking-tight leading-[1.1] mb-5 sm:mb-6 max-w-[800px]">
              {t('title')}
              <br />
            ...
            <Link href={`${localePath}/#contato` as never}
              className="inline-flex items-center gap-2 px-6 py-3 rounded-lg text-white font-semibold text-sm no-underline hover:-translate-y-0.5 transition-all duration-300"
        ...
        <section className="py-10 sm:py-14 relative">
          <div className="max-w-[1100px] mx-auto px-5 sm:px-6">
            <h2 className="text-xs uppercase tracking-[0.18em] mb-5 font-mono" style={{ color: 'var(--cyan)' }}>
              {tShared('stackTitle')}
            </h2>
        ...
        <section className="py-14 sm:py-20 relative">
          <div className="max-w-[860px] mx-auto px-5 sm:px-6">
            <h2 className="text-[clamp(1.4rem,3.5vw,2rem)] font-black tracking-tight mb-8">
            ...
            <div className="flex flex-col gap-3">
              {faqItems.map((it) => (
                <details key={it.q}
                  ...
                  <summary className="list-none flex items-start justify-between gap-4 font-semibold text-sm sm:text-base">
                    {it.q}
                    <ChevronRight size={18} className="flex-shrink-0 mt-0.5 transition-transform duration-300 group-open:rotate-90" color="var(--cyan)" />
                  </summary>
        ...
        <section className="py-16 sm:py-24 relative overflow-hidden">
          <div aria-hidden="true" className="absolute inset-0 pointer-events-none"
            style={{ background: 'radial-gradient(ellipse 70% 50% at 50% 50%, rgba(232,75,26,0.10) 0%, transparent 70%)' }} />
          <div className="max-w-[860px] mx-auto px-5 sm:px-6 text-center relative z-10">
            <h2 className="text-[clamp(1.6rem,4.5vw,2.4rem)] font-black tracking-tight mb-4">
              {tShared('ctaTitle')}
            </h2>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              <Link href={`${localePath}/#contato` as never}
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-lg text-white font-semibold text-sm no-underline hover:-translate-y-0.5 transition-all duration-300"
                style={{ background: 'linear-gradient(135deg, var(--cyan), var(--blue))', boxShadow: '0 4px 24px rgba(232,75,26,0.45)' }}>
                {tShared('ctaButton')}
                <ArrowRight size={16} strokeWidth={2.2} />
              </Link>
              <Link href={`${localePath}/` as never}
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-lg font-semibold text-sm no-underline hover:-translate-y-0.5 transition-all duration-300"
